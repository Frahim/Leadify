...
loginButton.addEventListener("click", async () => {
    ...
    try {
        const response = await fetch("https://adaptstoday.xyz/api/login", {
            method: "POST",
...
async function importLeads(token, leads, messageElement) {
    ...
    try {
        const response = await fetch("https://adaptstoday.xyz/api/leads/import", {
            method: "POST",
            headers: {
...
async function fetchExistingLeads(token, messageElement) {
    try {
        const response = await fetch("https://adaptstoday.xyz/api/all-leads", {
            headers: {
                Authorization: `Bearer ${token}`,
...
async function isLeadAlreadyImported(token, profileUrl) {
    try {
        ...
        const profileId = extractLinkedInId(profileUrl);
        if (!profileId) return false;

        const existingIds = leads
            .map(l => extractLinkedInId(l.url))
            .filter(Boolean);

        return existingIds.includes(profileId);
    } catch {
        return false;
    }
}
